...
def parseArgs():
    ...
    parser.add_argument('--imagedir', dest='imagedir', default='/home/y212202015/data/ADEChallengeData2016/images/validation', type=str)
    parser.add_argument('--imagepath', dest='imagepath', default='', type=str)
    parser.add_argument('--outputfilename', dest='outputfilename', default='/home/y212202015/SSEG/sseg/ADE20K_ResNet50_Visual', type=str)
...
class Demo():
    def __init__(self):
        ...
        logger_handle = logger_config(log_path="/home/y212202015/SSEG/sseg/val.log",logging_name="train.log")
    ...
    def start(self):
        ...
        segmentor = BuildSegmentor(segmentor_cfg=copy.deepcopy(cfg.SEGMENTOR_CFG), mode='TEST',logger_handle = self.logger_handle)
        ...
        for idxx in pbar:
            imagepath = imagepaths[idxx]
            pathh = imagepath.split("/")[-1].split(".")[0]
            ...
            if cmd_args.outputfilename:
                cv2.imwrite(os.path.join(cfg.COMMON_CFG['work_dir'],pathh + '.png'), image)
            else:
