...
class BaseSegmentor(nn.Module):
    ...
    def calculatelosses(self, predictions,loss_ce,loss_ppc,multiply_distance,loss_distance_weight,loss_ce_weight,loss_ppc_weight,targets, losses_cfg, map_preds_to_tgts_dict=None): #计算每个损失函数的值
        # parse targets
        ...
        loss = 0
        if loss_ppc_weight!=None:
            losses_log_dict['loss_ce'] = loss_ce * loss_ce_weight
            losses_log_dict['loss_ppc'] = loss_ppc * loss_ppc_weight
            losses_log_dict['loss_distance'] = loss_distance_weight * multiply_distance
        #losses_log_dict['loss_ppd'] = loss_ppd * 0.1
        #loss = losses_log_dict['loss_ppc']  + losses_log_dict['loss_ce']
    ...
    def calculateloss(self, prediction, target, loss_cfg):
        ...
        # prediction_format.size(-1)  -1表示最后一维的维度是多少
