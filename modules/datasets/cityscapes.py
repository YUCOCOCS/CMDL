...
class CityScapesDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'TRAIN':
            sample = self.synctransform(sample, 'without_totensor_normalize_pad') # 先不要转成tensor、正则化、填充
            # sample['edge'] = self.generateedge(sample['segmentation'].copy())
            sample = self.synctransform(sample, 'only_totensor_normalize_pad') #再转成tensor,正则化和填充
        else:
